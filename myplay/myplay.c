// /usr/bin/rsync -qavzhe ssh %{CurrentProject:Path}/*.cpp pi@raspi16:/home/pi/Raspi-Hifi-V1.0.2
// /usr/bin/ssh pi@raspi16 "cd /home/pi/Raspi-Hifi-V1.0.2;gcc myplay.c"
...
static void playback_go(int fd, size_t loaded, off64_t count, int rtype, char *name)
{
    ...
    if( fwd )
    {
      size_t bytespersec = rhwparams.channels * rhwparams.rate * bits_per_sample / 8;
      lseek(fd,10*bytespersec,SEEK_CUR);
      fwd = 0;
    }
    if( rev )
    {
